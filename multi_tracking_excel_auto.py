...
excel_time = 0
@jit
def compare (time,fps_count,excel_time):
    Rs_xmin = 0
    Rs_ymin = 0
    Rs_xmax = 0
    Rs_ymax = 0
    ...
    print("Video Time:"+str(time)+str(fps_count%(30/args["rate"])==0))
    print(excel_time)
    #compare time with vott csv file time and show on Terminal
    time_flag=df3[round(df3['Time'],1)== round(float(excel_time),1)]
    #print(time_flag)
...
while True:
    ...
    if fps_count%(30/args["rate"])==0:
        out.write(frame)
        excel_time+= 1/args["rate"]
    fps_count += 1
